...
- (BOOL)isInRange
{
	if( device && [device remoteNameRequest:nil 
							withPageTimeout:[pageTimeoutSlots intValue]] == kIOReturnSuccess )
		return true;
...
- (void)userDefaultsLoad
{
	...
	// Device
	if ([[defaults stringForKey:@"deviceAddress"] length] > 0) {
		BluetoothDeviceAddress deviceAddress;
		IOBluetoothNSStringToDeviceAddress([defaults stringForKey:@"deviceAddress"], &deviceAddress);
		device = [IOBluetoothDevice withAddress:&deviceAddress];
	} else {
	...
	// Page timeout in slots.  1600 slots in 1 second; default is 8192 (5.12 seconds).
	if ([[defaults stringForKey:@"pageTimeoutSlots"] length] > 0)
		[pageTimeoutSlots setStringValue:[defaults stringForKey:@"pageTimeoutSlots"]];
	else 
		[pageTimeoutSlots setStringValue:@"8192"];
