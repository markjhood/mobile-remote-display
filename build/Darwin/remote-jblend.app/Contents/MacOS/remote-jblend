...
# hood - Original didn't work with spaces in the path.  
#name="`basename $0`"
#tmp="`pwd`/$0"
#
# hood - Following should work as long as the script is invoked at the level
# of the app bundle directory or above, and with a modified exec call at the
# end.
name="$0"
tmp=`dirname "$0"`
tmp=`dirname "$tmp"`
...
# hood - get default args from user db if they exist
DEFAULTS=
ipaddr=`defaults read com.aplix.RemoteDisplay ipaddr`
if [ -n "$ipaddr" ]; then
  DEFAULTS="-c $ipaddr "
fi
port=`defaults read com.aplix.RemoteDisplay port`
if [ -n "$port" ]; then
  DEFAULTS="$DEFAULTS -p $port "
fi
mode=`defaults read com.aplix.RemoteDisplay demoMode`
if [ -n "$mode" ]; then
  DEFAULTS="$DEFAULTS -s $mode "
fi

# hood - define image directory.  
export BUNDLE_IMGDIR="$bundle_res"/images

# hood -- Put EXTRA_ARGS before command line args so that EXTRA_ARGS can be
# overridden.
$EXEC "$name-bin" $DEFAULTS $EXTRA_ARGS $*
